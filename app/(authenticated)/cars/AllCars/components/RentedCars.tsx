  ...
  return (
    ...
    <div className="w-[350px]">
      ...
      {rentedCars.length === 0 ? (
        ...
        <h6 className="text-sm text-gray-400">You haven&apos;t rented any cars yet.</h6>
        <Button>Book a Car!</Button>
      </div>
      ) : (
        <div className="flex flex-col justify-center items-center">
          {rentedCars.map((rentedCar, index) => (
            <div key={index} className="w-[350px] p-4 shadow-md rounded-lg border border-primary flex items-center gap-4">
              ...
              <div className="flex flex-col flex-grow gap-1">
                <h2 className="text-lg font-semibold text-primary">{rentedCar.cars.brand}</h2>
                <div className="flex justify-between items-center ">
                  <div className="flex justify-center items-center gap-1">
                    <Fuel className="w-3 h-3 text-primary" />
                    <p className="text-sm text-white/50 min-w-10">{rentedCar.cars.mileage} L</p>
                  </div>
                  <div className="flex justify-center items-center gap-1">
                    <KeySquare className="w-3 h-3 text-primary" />
                    <p className="text-sm text-white/50 min-w-14">{rentedCar.cars.transmission}</p>
                  </div>
                  <div className="flex justify-center items-center gap-1">
                    <Calendar className="w-3 h-3 text-primary" />
                    <p className="text-sm text-white/50 min-w-10">{rentedCar.cars.year}</p>
                  </div>
                  <div className="flex justify-center items-center gap-1">
                    <Armchair className="w-3 h-3 text-primary" />
                    <p className="text-sm text-white/50 min-w-10">{rentedCar.cars.no_seats}</p>
                  </div>
                </div>
                ...
                <p className="text-sm text-white/40">
                  Rented till:{" "}
                  {new Date(rentedCar.rents.endDate).toLocaleDateString("en-GB", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })
